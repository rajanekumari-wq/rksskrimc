...
function SearchResultsSection({ query, onClose }: { query: string, onClose: () => void }) {
  const [activeTab, setActiveTab] = useState<'all' | 'creators' | 'vibes'>('all');
  ...
  const autocompleteSuggestions = React.useMemo(() => {
    ...
    const q = query.toLowerCase();
    const suggestions: { type: string, label: string, name?: string, image?: string }[] = [];
        ...
        {activeTab === 'all' && query.trim() && autocompleteSuggestions.length > 0 && (
          <div className="flex flex-col mb-6 bg-white/5 rounded-xl overflow-hidden border border-white/5">
             {autocompleteSuggestions.map((s, i) => (
                <div key={i} className="flex items-center gap-3 p-3 border-b border-white/5 hover:bg-white/10 cursor-pointer" onClick={() => navigate(s.type === 'hashtag' ? `/hashtag/${encodeURIComponent(s.label)}` : s.type === 'creator' ? `/profile/${s.label.replace('@', '')}` : `/search?q=${s.label}`)}>
              ...
              <div>
                ...
                <div className="flex flex-col gap-1">
                  {recentSearches.map(term => (
                    <div 
                      key={term} 
                      onClick={() => navigate(`/hashtag/${encodeURIComponent(term)}`)}
          ...
          <div className="flex flex-col gap-6 pb-6">
                           ...
                           {hashtags.length > 0 && (
                             <div className="flex items-center gap-1 flex-wrap">
                               {hashtags.slice(0,3).map((h: string, idx: number) => (
                                 <span key={idx} className="text-[#00F0FF] text-[10px] font-bold">{h}</span>
                               ))}
                             </div>
                           )}
                        </div>
            ...
            {activeTab === 'vibes' && vibeResults.length === 0 && (
               <div className="py-20 flex flex-col items-center justify-center text-center px-4">
                  ...
                  <button onClick={() => { setActiveTab('vibes'); setCreatorResults([]); setVibeResults([]); onClose(); }} className="bg-white/10 hover:bg-white/20 text-white font-bold px-6 py-3 rounded-full border border-white/20 transition-all text-sm flex items-center gap-2">
...
export default function DiscoverScreen() {
  ...
  return (
    ...
    >
      ...
      {searchFocused ? (
        <SearchResultsSection query={query} onClose={() => {setQuery(""); setSearchFocused(false);}} />
      ) : (
        <div className="pt-6">
