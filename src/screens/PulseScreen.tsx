...
export default function PulseScreen() {
  ...
  const groupedSparks = React.useMemo(() => {
    ...
    sparks.forEach(spark => {
      ...
      const isOwn = spark.isOwn || (spark.user?.username && currentUser && spark.user?.username === currentUser?.username);
      
      // Default processor
      processUserSparks(isOwn ? (currentUser || spark.user) : spark.user, isOwn);
