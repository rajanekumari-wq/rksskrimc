...
const BUBBLE_COLORS = [
  { id: 'purple', label: 'Purple', gradient: ['#7B2FF7', '#B026FF'], preview: '#9B26FF' },
  { id: 'pink', label: 'Pink', gradient: ['#FF1493', '#FF69B4'], preview: '#FF1493' },
  { id: 'blue', label: 'Blue', gradient: ['#006994', '#00B4FF'], preview: '#00B4FF' },
  { id: 'green', label: 'Green', gradient: ['#006400', '#00C853'], preview: '#00C853' },
  { id: 'gold', label: 'Gold', gradient: ['#B8860B', '#FFD700'], preview: '#FFD700' },
  { id: 'red', label: 'Red', gradient: ['#8B0000', '#FF4500'], preview: '#FF4500' }
];
...
const SOLID_COLORS = [
  '#000000', '#1A1A24', '#2C3E50', '#8E44AD', '#2980B9', '#27AE60', '#D35400', '#C0392B',
  '#F39C12', '#16A085', '#34495E', '#7F8C8D', '#BDC3C7', '#ecf0f1', '#95a5a6'
];

export function ChatCustomizationSheet({ onClose, onApply, onReset, initialSettings, originalName }: Props) {
  const [settings, setSettings] = useState<CustomizationSettings>(initialSettings);
  const [editingNickname, setEditingNickname] = useState(false);
  const [nicknameInput, setNicknameInput] = useState(settings.nickname || originalName);
    ...
    return (
      <div className="relative w-full h-[180px] rounded-2xl overflow-hidden border border-white/10 mb-6 bg-[#0A0A0F]">
         ...
         {settings.wallpaper.startsWith('#') ? (
            <div className="absolute inset-0" style={{ backgroundColor: settings.wallpaper }} />
         ) : CHAT_THEMES.find(t => t.id === settings.wallpaper) ? (
            <div 
               className="absolute inset-0 opacity-40 transition-colors duration-1000" 
               style={{ backgroundColor: CHAT_THEMES.find(t => t.id === settings.wallpaper)?.preview || '#1A1A24' }} 
            />
         ) : settings.wallpaper.startsWith('http') || settings.wallpaper.startsWith('data:') ? (
            <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${settings.wallpaper})` }}>
         ...
         <div className={`relative z-10 p-4 flex flex-col gap-3 justify-end h-full`}>
            {/* Their msg */}
            <div className="flex self-start max-w-[80%]">
               <div className={`px-4 py-2 ${settings.textSize === 'small' ? 'text-xs' : settings.textSize === 'large' ? 'text-lg' : 'text-[15px]'} ${settings.font === 'mono' ? 'font-mono' : 'font-sans'} text-white break-words shadow-lg
                  ${settings.font === 'rounded' ? 'rounded-[20px] rounded-bl-[4px]' : 'rounded-2xl rounded-bl-[4px]'}
               `}
               style={{
                  background: settings.theirBubbleStyle === 'dark' ? 'rgba(30,30,30,1)' : 
                              settings.theirBubbleStyle === 'tinted' ? (settings.bubbleColor.startsWith('#') ? `${settings.bubbleColor}30` : `${BUBBLE_COLORS.find(c => c.id === settings.bubbleColor)?.preview}30`) : 
                              'rgba(255,255,255,0.08)',
                  backdropFilter: settings.theirBubbleStyle === 'glass' ? 'blur(12px)' : 'none',
                  border: settings.theirBubbleStyle === 'glass' ? '1px solid rgba(255,255,255,0.1)' : 'none'
               }}
               >
                 Hey! How are you?
               </div>
            ...
            <div className="flex self-end max-w-[80%]">
               <div className={`px-4 py-2 ${settings.textSize === 'small' ? 'text-xs' : settings.textSize === 'large' ? 'text-lg' : 'text-[15px]'} ${settings.font === 'mono' ? 'font-mono' : 'font-sans'} text-white shadow-[0_4px_12px_rgba(0,0,0,0.1)]
     ...
     return (
        <div className="fixed inset-0 z-[100] bg-[#0A0A0F] flex flex-col pt-safe">
           <div className="flex items-center gap-3 p-4 border-b border-white/5">
              <button onClick={() => setShowWallpaperPicker(false)} className="text-white hover:text-white/70">
                 <ArrowLeft size={24} />
              </button>
           ...
           <div className="p-4 overflow-y-auto pb-safe">
                       ...
                       if (file) {
                          ...
                          reader.onload = (event) => {
                             if (event.target?.result) {
                                setSettings({ ...settings, wallpaper: event.target.result as string });
              ...
              <div className="text-xs font-bold text-white/50 mb-3 mt-6">SKRIMCHAT WALLPAPERS</div>
              ...
              <div className="text-xs font-bold text-white/50 mb-3">SOLID COLORS</div>
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#0A0A0F] flex flex-col pt-safe">
       ...
       <div className="flex-1 overflow-y-auto p-4 pb-24">
          <div className="text-[10px] font-bold text-white/40 mb-2 tracking-wider">LIVE PREVIEW</div>
          {renderLivePreview()}

          <div className="bg-[#1A1A24] rounded-2xl p-4 mb-4 border border-white/5">
             <div className="text-[10px] font-bold text-white/40 mb-3 tracking-wider">NICKNAME</div>
             {editingNickname ? (
                <div className="bg-black/20 p-3 rounded-xl border border-white/10">
                   <div className="flex items-center gap-2 mb-2">
                      ...
                      />
                      <button onClick={() => {
                         setSettings({ ...settings, nickname: nicknameInput || originalName });
                         setEditingNickname(false);
                      }} className="w-8 h-8 rounded-full bg-white text-black flex items-center justify-center">
                         <Check size={16} />
                      </button>
                   </div>
                   <div className="text-[11px] text-white/40 mb-3">Only visible to you. {originalName} sees her real name.</div>
                   <div className="flex gap-2 mb-3">
                      {["Bestie 💜", "Bro 🤝", "Queen 👑", "Boss 😎"].map(suggest => (
                         <button key={suggest} onClick={() => setNicknameInput(suggest)} className="bg-white/5 px-2 py-1 rounded-md text-[11px] text-white/70 hover:text-white">
                            {suggest}
                         </button>
                      ))}
                   </div>
                ...
                <div className="flex items-center justify-between">
                   <div>
                      <div className="text-white font-medium text-[15px]">{settings.nickname || originalName}</div>
                      <div className="text-[11px] text-white/40">Only you see this name</div>
                   </div>
                   <button onClick={() => setEditingNickname(true)} className="p-2 text-white/60 hover:text-white bg-white/5 rounded-full">
                      <Edit2 size={16} />
                   </button>
                </div>
             )}
          ...
          <div className="bg-[#1A1A24] rounded-2xl p-4 mb-4 border border-white/5">
             <div className="text-[10px] font-bold text-white/40 mb-3 tracking-wider">WALLPAPER</div>
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                   <div 
                      className="w-10 h-10 rounded-lg overflow-hidden flex items-center justify-center bg-[#0A0A0F] border border-white/20"
                      style={
                         settings.wallpaper.startsWith('#') ? { backgroundColor: settings.wallpaper } :
                         CHAT_THEMES.find(t => t.id === settings.wallpaper) ? { backgroundColor: CHAT_THEMES.find(t => t.id === settings.wallpaper)?.preview } :
                         (settings.wallpaper.startsWith('http') || settings.wallpaper.startsWith('data:')) ? { backgroundImage: `url(${settings.wallpaper})`, backgroundSize: 'cover' } :
                         {}
                      }
                   >
                   ...
                   <div className="text-sm text-white/70">
                      {CHAT_THEMES.find(t => t.id === settings.wallpaper)?.name || (settings.wallpaper.startsWith('#') ? 'Solid Color' : 'Custom Image')}
                   </div>
                </div>
                <button onClick={() => setShowWallpaperPicker(true)} className="text-xs font-bold px-3 py-1.5 bg-white/10 rounded-full text-white/80 hover:text-white">
                   🎨 Change
                </button>
             </div>
          </div>
          ...
          <div className="bg-[#1A1A24] rounded-2xl p-4 mb-4 border border-white/5">
             <div className="text-[10px] font-bold text-white/40 mb-3 tracking-wider">BUBBLE COLORS</div>
             ...
             <div className="flex items-center gap-2 mb-4 overflow-x-auto hide-scrollbar">
                {BUBBLE_COLORS.map(color => (
                   <button 
                      key={color.id}
                      onClick={() => setSettings({ ...settings, bubbleColor: color.id })}
                ...
                <div className="bg-black/20 p-4 rounded-xl border border-white/10 mb-4 animate-fade-in">
                   <div className="flex justify-between items-center mb-3">
                      ...
                      <button 
                         ...
                         onClick={() => {
                            setSettings({ ...settings, bubbleColor: `hsl(${customHue}, 80%, 50%)` });
                   ...
                   />
                   <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full" style={{ backgroundColor: `hsl(${customHue}, 80%, 50%)` }} />
             ...
             <div className="flex items-center gap-2">
                {THEIR_STYLES.map(style => (
                   <button 
                      key={style.id}
                      onClick={() => setSettings({ ...settings, theirBubbleStyle: style.id })}
          ...
          <div className="bg-[#1A1A24] rounded-2xl p-4 mb-4 border border-white/5">
             <div className="text-[10px] font-bold text-white/40 mb-3 tracking-wider">TEXT SIZE</div>
          ...
          <div className="bg-[#1A1A24] rounded-2xl p-4 mb-6 border border-white/5">
             <div className="text-[10px] font-bold text-white/40 mb-3 tracking-wider">CHAT FONT</div>
             <div className="flex items-center bg-black/20 rounded-xl p-1 border border-white/5">
                {FONTS.map(font => (
                   <button 
                      key={font.id}
          ...
          <div className="flex justify-center pb-8">
             {showResetConfirm ? (
                <div className="flex flex-col items-center">
                   <div className="text-white/70 text-sm mb-3 text-center max-w-[200px]">Reset all customizations for this chat? This cannot be undone.</div>
                   <div className="flex gap-3">
                      <button onClick={onReset} className="px-4 py-1.5 bg-[#FF3B30] text-white text-xs font-bold rounded-full hover:opacity-80">Reset</button>
                      <button onClick={() => setShowResetConfirm(false)} className="px-4 py-1.5 bg-white/10 text-white text-xs font-bold rounded-full hover:bg-white/20">Cancel</button>
                   </div>
                </div>
             ) : (
                <button onClick={() => setShowResetConfirm(true)} className="flex items-center gap-2 text-white/40 hover:text-white text-sm">
                   ↩️ Reset to Default
                </button>
             )}
          </div>
