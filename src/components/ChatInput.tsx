...
export function ChatInput({ currentMood, onSetMood, onSendMessage, onSendVoice, onSendGif, onSendSticker, onSendAttachment, onSendPoll, onOpenGamePicker, onTyping, replyingTo, onCancelReply, smartReplies, onDismissSmartReplies }: Props) {
      ...
      <AnimatePresence>
        {showAttachMenu && (
          <AttachmentPicker 
            ...
            onSendPoll={(data) => { onSendPoll?.(data); setShowAttachMenu(false); }}
            ...
            >
              <div className="flex items-center justify-between w-full px-2 mb-1.5">
                 <span className="text-[10px] font-bold text-[#B026FF] flex items-center gap-1 drop-shadow-md">
                   ⚡ SkrimAI
                   {showHoldTooltip && <span className="text-white/50 ml-1 font-normal animate-pulse">(Hold to edit before sending)</span>}
                 </span>
                 <button onClick={onDismissSmartReplies} className="text-white/40 hover:text-white text-[10px] uppercase font-bold px-2 py-0.5 bg-white/5 rounded-full">✕ Dismiss</button>
              </div>
                      ...
                      onPointerDown={(e) => {
                         holdTimerRef.current = setTimeout(() => {
                            ...
                            if (showHoldTooltip) {
                               ...
                               localStorage.setItem('skrim_smart_reply_tooltip_shown', 'true');
                            }
