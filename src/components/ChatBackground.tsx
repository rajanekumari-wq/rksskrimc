...
export function ChatBackground({ wallpaper, mood, energyLevel = 0, energyOn = true }: Props) {
  const currentMoodObj = CHAT_MOODS.find(m => m.id === mood);
  
  // Decide what type of wallpaper it is
  const isSolidOrGradient = wallpaper.startsWith('#') || wallpaper.startsWith('linear-gradient') || wallpaper.startsWith('hsl');
  const isImage = wallpaper.startsWith('http') || wallpaper.startsWith('data:');
  const themeObj = CHAT_THEMES.find(t => t.id === wallpaper) || CHAT_THEMES[0];
  ...
  return (
    <div 
      ...
      style={
         isSolidOrGradient ? { background: wallpaper } :
         isImage ? { backgroundImage: `url(${wallpaper})`, backgroundSize: 'cover', backgroundPosition: 'center' } :
         { backgroundColor: themeObj.preview, color: themeObj.textColor || 'white' }
      }
    >
      ...
      {/* Special Particles */}
      {isTheme && themeObj.particles === 'stars' && energyOn && particles.map(p => (
         <motion.div 
            key={p.id}
      ...
      {isTheme && themeObj.particles === 'diyas' && energyOn && particles.map(p => (
         <motion.div 
            key={p.id}
      ...
      {isTheme && themeObj.particles === 'raindrops' && energyOn && particles.map(p => (
         <motion.div 
            key={p.id}
